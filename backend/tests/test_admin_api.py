...
def test_login_failure(mock_auth_user, client):
    ...
    response = client.post("/admin/login", json={"username": "admin", "password": "wrong_password"})
...
def test_upload_empty_file(client, mock_admin_user):
    ...
    response = client.post("/admin/upload?language_set_id=1", files={"file": ("empty.csv", "", "text/csv")})
...
def test_upload_non_csv_file(client, mock_admin_user):
    ...
    response = client.post(
        "/admin/upload?language_set_id=1", files={"file": ("test.txt", "not csv content", "text/plain")}
    )
...
def test_get_user_by_id(mock_get_account, client, mock_root_admin_user):
    ...
    # Override the dependency
    app.dependency_overrides[require_admin_access] = lambda: mock_root_admin_user
    mock_get_account.return_value = {"id": 1, "username": "admin", "role": "administrative", "is_active": True}
...
def test_create_user_already_exists(mock_get_by_username, client, mock_root_admin_user):
    ...
    response = client.post("/admin/users", json={"username": "existing", "password": "password123", "role": "regular"})
...
def test_create_user_invalid_role(client, mock_root_admin_user):
    ...
    response = client.post(
        "/admin/users", json={"username": "newuser", "password": "password123", "role": "invalid_role"}
    )
...
def test_get_profile_regular_user(mock_get_account, client, mock_regular_user):
    ...
    mock_get_account.return_value = {"id": 2, "username": "user", "role": "regular", "self_description": "Regular user"}
...
def test_update_profile(mock_update_account, client, mock_regular_user):
    ...
    response = client.put("/admin/profile", json={"self_description": "Updated description"})
...
def test_update_profile_empty_description(client, mock_regular_user):
    ...
    response = client.put("/admin/profile", json={"self_description": "   "})
